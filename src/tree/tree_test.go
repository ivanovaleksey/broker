...
func TestTree_AddSubscription(t *testing.T) {
	t.Run("initial tree idea", func(t *testing.T) {
		...
		t.Run("hash case", func(t *testing.T) {
			...
			assert.Equal(t, node.NodeTypeHash, firstHashNode.Type)
			...
			assert.Equal(t, node.NodeTypeWord, oneNode.Type)
			...
			assert.Equal(t, node.NodeTypeHash, secondHashNode.Type)
			...
			assert.Equal(t, node.NodeTypeWord, twoNode.Type)
			...
			assert.Equal(t, node.NodeTypeHash, thirdHashNode.Type)
		...
		t.Run("logs and events case", func(t *testing.T) {
			...
			assert.Equal(t, node.NodeTypeWord, debugNode.Type)
			...
			assert.Equal(t, node.NodeTypeWord, eventsNode.Type)
			...
			assert.Equal(t, node.NodeTypeWord, ordersNode.Type)
			...
			assert.Equal(t, node.NodeTypeStar, ordersStarNode.Type)
			...
			assert.Equal(t, node.NodeTypeHash, ordersHashNode.Type)
	...
	t.Run("plain tree", func(t *testing.T) {
		t.Run("hash case", func(t *testing.T) {
...

			{
				...
				assert.Equal(t, node.NodeTypeHash, hashNode.Type)
				...
				assert.Equal(t, node.NodeTypeHash, oneHashNode.Type)
				...
				assert.Equal(t, node.NodeTypeHash, twoHashNode.Type)
		...
		t.Run("hash.star combination", func(t *testing.T) {
			...
			t.Run("hash and dot", func(t *testing.T) {
				...
				require.NotNil(t, secondHashNode)
				require.Equal(t, 0, len(secondHashNode.Next))
				assert.Equal(t, node.NodeTypeStar, secondHashNode.Type)
				assert.Equal(t, true, secondHashNode.Stop)
				secondHashNodeConsumers := tree.nodeConsumers.GetConsumers(secondHashNode.ID)
				assert.Equal(t, []int64{consumerB}, mapToSliceHelper(secondHashNodeConsumers))
			})
